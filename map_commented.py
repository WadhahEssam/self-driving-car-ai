...
class Car(Widget):
    ...
    sensor1 = ReferenceListProperty(sensor1_x, sensor1_y) # first sensor vector will detect if there is any sand in front of the car.
    ...
    sensor2 = ReferenceListProperty(sensor2_x, sensor2_y) # second sensor vector will detect the left of the car
    ...
    sensor3 = ReferenceListProperty(sensor3_x, sensor3_y) # third sensor vector will detect the right of the car
    signal1 = NumericProperty(0) # initializing the signal received by sensor 1 ( the density of sand around signal one ) 
    signal2 = NumericProperty(0) # initializing the signal received by sensor 2 ( the density of the sand around signal two )
    signal3 = NumericProperty(0) # initializing the signal received by sensor 3 ( the density of the sand around signal three )

    # how does it detect the density of sand 
    # it takes a square infront of the sensor and 
    # it divides the number of squares with one inside 
    # them ( the sand )o ver the number of 
    # squares with zero inside them

    # will allow the car to move to any direction
    def move(self, rotation):
        ...
        # after you change the direction of the car you will need to change the direction of the sensors and the signals
        ...
        # Here you are punishing the car for reaching outside of the box
        # so the car will tetect that the corners of the map are at full 
        # sand . remember that the signal is a number between 0 and 1 
        # which represents the percentage of sand infront of the sensors
...
class Game(Widget):
    ...
    def update(self, dt): # the big update function that updates everything that needs to be updated at each discrete time t when reaching a new state (getting new signals from the sensors)
        ...
        ## those are the values that will be sent to our ai as the Input 
        # and it sent it the values of orientation and the minus orentation 
        # because it helps with the exploration vs exploiation 
        ...
        ## this is what our neural network AI will return
        ...
        # this will slow the car down if it is on sand
        # so if it goes into sand the reward that it will get is 
        # minus one which is the worst reward that it can get 
        # and if it is not moving above sand it will get a slightly 
        # better reward which is a pus one reward
        # but then we also needed to check weather the car is going 
        # closer to the goal or farther from the goal, and if you 
        # you are going farther from the goal the reward will be 
        # decreased slighly
...
# this class is not really important it is just for painting the road and 
# other objects on the map and this is related to kivy
...
# those are to add the api buttons 
class CarApp(App):
